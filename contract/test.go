...
func GenerateTestContext(t *testing.T, value *big.Int, address common.Address, params ...interface{}) (*state.StateDB, *ModuleContract) {
	var (
		block     = int(0)
	...
	ctx := NewModuleContract(sdb, contractRef)
...
func TestModuleCall(t *testing.T, contract common.Address, name string, payload []byte, value *big.Int, params ...interface{}) ([]byte, error) {
	_, ctx := GenerateTestContext(t, value, contract, params...)
	ref := ctx.ContractRef()
	res, _, err := ref.ModuleCall(ref.caller, contract, payload)
	if DebugSpentOpen {
