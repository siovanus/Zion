...
func (h *Handler) MakeDepositProposal(service *contract.ModuleContract) (txParam *common2.MakeTxParam, err error) {
	...
	err = common2.CheckDoneTx(service, txParam.CrossChainID, params.SourceChainID)
	if err != nil {
		err = fmt.Errorf("eth common handler check done transaction err: %v, chain(%d): %s", err, params.SourceChainID, sideChain.Name)
		return
	}

	err = common2.PutDoneTx(service, txParam.CrossChainID, params.SourceChainID)
	if err != nil {
...
func (h *Handler) VerifyDepositProposal(service *contract.ModuleContract,
	...
	txParam, err = common2.DecodeTxParam(params.Extra)
	return
}
...
func VerifyCrossChainProof(value []byte, proof *Proof, root common.Hash, address []byte) (err error) {
	...
	for _, s := range proof.AccountProof {
		nodeList.Put(nil, common.Hex2Bytes(common2.Replace0x(s)))
	}
	ns := nodeList.NodeSet()
	addr := common.Hex2Bytes(common2.Replace0x(proof.Address))
	...
	nonce, ok := new(big.Int).SetString(common2.Replace0x(proof.Nonce), 16)
	if !ok {
		err = fmt.Errorf("invalid account nonce: %s", proof.Nonce)
		return
	}
	balance, ok := new(big.Int).SetString(common2.Replace0x(proof.Balance), 16)
	if !ok {
	...
	for _, p := range sp.Proof {
		nodeList.Put(nil, common.Hex2Bytes(common2.Replace0x(p)))
	}
